...
class murray_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'murray_rp', // Base ID
			__('Murray Recent Posts with Thumbnails', 'murray'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'murray' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'murray' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'murray' );
		}
		?>
		<p>
...
add_action( 'widgets_init', 'murray_register_widget' );  
function murray_register_widget() {  
    register_widget( 'murray_Recent_Posts' );  
}  
